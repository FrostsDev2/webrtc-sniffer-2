// ============================================================
//  🐉 Frosts WebRTC Peer Inspector v0.1.4
// ============================================================
...
const themes = {
  midnight: { bg:'#0d0d1a', border:'#7b68ee', text:'#c8b8ff', sub:'#9988cc', dim:'#443366', header:'#0a0520', name:'🌙 Midnight', gradient:'linear-gradient(135deg,#0a0520,#1a1040)' },
  matrix:   { bg:'#0a0a0a', border:'#00ff88', text:'#00ff88', sub:'#aaffcc', dim:'#446644', header:'#001a00', name:'💚 Matrix',   gradient:'linear-gradient(135deg,#001a00,#002a00)' },
  blood:    { bg:'#0a0000', border:'#ff2222', text:'#ff6666', sub:'#cc4444', dim:'#441111', header:'#1a0000', name:'🔴 Blood',    gradient:'linear-gradient(135deg,#1a0000,#2a0000)' },
  ice:      { bg:'#0a0f1a', border:'#00bfff', text:'#aaddff', sub:'#88bbdd', dim:'#224466', header:'#001133', name:'🧊 Ice',      gradient:'linear-gradient(135deg,#001133,#002244)' },
  gold:     { bg:'#0f0a00', border:'#ffaa00', text:'#ffdd88', sub:'#ccaa44', dim:'#443300', header:'#1a0f00', name:'👑 Gold',     gradient:'linear-gradient(135deg,#1a0f00,#2a1a00)' },
  rose:     { bg:'#0f0a0d', border:'#ff69b4', text:'#ffb6d9', sub:'#cc88aa', dim:'#553344', header:'#1a0010', name:'🌸 Rose',     gradient:'linear-gradient(135deg,#1a0010,#2a0020)' },
  cyber:    { bg:'#050510', border:'#00ffff', text:'#00ffff', sub:'#88ffff', dim:'#224444', header:'#001a1a', name:'⚡ Cyber',    gradient:'linear-gradient(135deg,#001a1a,#002828)' },
  ember:    { bg:'#100800', border:'#ff6600', text:'#ffaa44', sub:'#cc7722', dim:'#442200', header:'#1a0800', name:'🔥 Ember',    gradient:'linear-gradient(135deg,#1a0800,#2a1000)' },
};
...
const settings = {
  ...
  showTimeline:   { val:true,  label:'📈 Timeline Bar',        desc:'Show visual timeline bar per peer.' },
...
styleTag.textContent = `
  ...
  #ppBody::-webkit-scrollbar,#tabSettings::-webkit-scrollbar,#tabStats::-webkit-scrollbar { width:3px; }
  #ppBody::-webkit-scrollbar-thumb,#tabSettings::-webkit-scrollbar-thumb,#tabStats::-webkit-scrollbar-thumb { background:#333; border-radius:2px; }
  ...
  #frostEmbedContainer::-webkit-scrollbar { width:3px; }
  #frostEmbedContainer::-webkit-scrollbar-thumb { background:#2a1a4a;border-radius:2px; }
  .embedPeerCard {
  ...
  .embedMapEl { border-radius:8px;overflow:hidden; }
  .embedMapEl .leaflet-container { background:#050510!important; }
  #frostEmbedHeader {
...
loader.innerHTML = `
  ...
  <div style="font-size:clamp(9px,2.5vw,12px);color:#443366;letter-spacing:2px;margin-bottom:36px;">VERSION 0.1.4</div>
...
function buildToggleHTML(key) {
  ...
  return `<div style="display:flex;justify-content:space-between;align-items:center;padding:clamp(8px,2.5vw,12px) 0;border-bottom:1px solid #161616;">
...
function buildThemeHTML() {
  return `
    <div style="color:#443366;font-size:clamp(8px,2vw,10px);letter-spacing:2px;margin-bottom:10px;font-weight:600;">THEME</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:18px;">
      ${Object.entries(themes).map(([key,t])=>`
        <button class="themeBtn" data-theme="${key}" style="
          background:${t.gradient};border:1px solid ${t.border}${themes[key]===currentTheme?'':'55'};
          color:${t.text};border-radius:10px;padding:8px 10px;cursor:pointer;
          font-size:clamp(9px,2.2vw,11px);font-family:inherit;
          opacity:${themes[key]===currentTheme?'1':'0.65'};
          box-shadow:${themes[key]===currentTheme?`0 0 12px ${t.border}44`:''};
...
panel.innerHTML=`
  <div id="pph" style="padding:clamp(10px,3vw,14px) clamp(12px,3.5vw,16px);background:linear-gradient(135deg,#0a0520,#120830);border-bottom:1px solid #2a1a4a;display:flex;justify-content:space-between;align-items:center;cursor:grab;user-select:none;flex-shrink:0;">
    <div>
      ...
      <div style="font-size:clamp(8px,2vw,10px);color:#443366;margin-top:3px;letter-spacing:1px;">v0.1.4 • WebRTC Peer Inspector</div>
    </div>
    <div style="display:flex;gap:clamp(3px,1.5vw,5px);align-items:center;">
      <button id="ppEmbedBtn" style="background:linear-gradient(135deg,#1a0f40,#2a1a60);border:1px solid #7b68ee;color:#c8b8ff;cursor:pointer;font-size:clamp(9px,2.5vw,11px);padding:clamp(4px,1.5vw,7px) clamp(6px,2vw,10px);border-radius:8px;font-family:inherit;font-weight:600;white-space:nowrap;">🔗 Embed</button>
  ...
  <div id="tabAbout" style="display:none;padding:clamp(16px,5vw,24px) clamp(14px,4vw,20px);flex:1;overflow-y:auto;">
    <div style="text-align:center;margin-bottom:clamp(14px,4vw,22px);">
      ...
      <div style="font-size:clamp(9px,2.2vw,11px);color:#443366;margin-top:5px;letter-spacing:1px;">WebRTC Peer Inspector • v0.1.4</div>
    </div>
...
function buildEmbedContainer() {
  ...
  el.style.cssText = `
    width:100%;background:${t.bg};border-top:2px solid ${t.border};
    padding:10px;font-family:'SF Mono','Fira Code',monospace;
    overflow-y:auto;max-height:400px;
  `;
  el.innerHTML = `
    <div id="frostEmbedHeader" style="background:${t.gradient};border:1px solid ${t.border}44;">
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="font-size:18px;">🐉</span>
        <div>
          <div style="font-weight:700;font-size:13px;color:${t.text};">Frosts Tools</div>
          <div style="font-size:9px;color:${t.dim};letter-spacing:1px;">v0.1.4 • EMBEDDED</div>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;">
        ...
        <button id="embedCopyAll" style="background:${t.header};border:1px solid ${t.border}44;color:${t.text};cursor:pointer;font-size:11px;padding:4px 8px;border-radius:6px;font-family:inherit;">📋 Copy</button>
        <button id="embedClose" style="background:${t.header};border:1px solid ${t.border}44;color:${t.dim};cursor:pointer;font-size:13px;padding:4px 8px;border-radius:6px;">✕</button>
      </div>
    </div>

    <div id="embedMapEl" class="embedMapEl" style="width:100%;height:160px;background:${t.header};border:1px solid ${t.border}22;border-radius:8px;margin-bottom:8px;display:flex;align-items:center;justify-content:center;">
      <div style="color:${t.dim};font-size:11px;">🗺️ Map loads when peer connects...</div>
    </div>

    <div id="embedPeerArea" style="display:flex;flex-direction:column;gap:6px;">
      <div id="embedEmpty" style="color:${t.dim};text-align:center;padding:16px 0;font-size:11px;">
...
function initEmbedMap() {
  ...
  script.onload=setupEmbedMap;document.head.appendChild(script);
}

function setupEmbedMap() {
  const mapEl=document.getElementById('embedMapEl');
  if (!mapEl) return;
  mapEl.innerHTML='';
...
function addEmbedMapMarker(p, fly=true) {
  ...
  const icon=L.divIcon({className:'',html:`<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2px solid #fff;box-shadow:0 0 8px ${color};"></div>`,iconSize:[14,14],iconAnchor:[7,7]});
...
function addEmbedPeerCard(p) {
  ...
  if (!settings.showAll.val) {
    area.innerHTML='';
    const countEl=document.getElementById('embedPeerCount');
    if (countEl) countEl.textContent='Peers: 0';
  }

  const t=currentTheme;
  const isTor=torExits.has(p.ip);
  const vpnColor=settings.highlightVPN.val&&(p.type.includes('🔴')||isTor)?'#ff4444':t.text;
  const color=getMarkerColor(p);
  const seenCount=getSeenCount(p.ip);

  const card=document.createElement('div');
  card.className='embedPeerCard';
  card.style.cssText=`background:${t.header};border:1px solid ${t.border}33;border-left:3px solid ${color};padding:10px 12px;`;
  card.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
      <div style="display:flex;align-items:center;gap:6px;">
        ${p.flag?`<img src="${p.flag}" style="width:16px;height:12px;border-radius:2px;">`:''}
        <b style="color:${t.text};font-size:12px;">${p.label} ${p.ip}${settings.showPort.val&&p.port?`<span style="color:${t.dim};font-size:10px;">:${p.port}</span>`:''}</b>
      </div>
      <div style="display:flex;gap:4px;align-items:center;">
        ${seenCount>0&&settings.showRepeat.val?`<span style="background:#ffaa0022;border:1px solid #ffaa00;color:#ffaa00;border-radius:4px;padding:1px 5px;font-size:9px;">🔁 ${seenCount}x</span>`:''}
        ${isTor&&settings.showTor.val?`<span style="background:#ff880022;border:1px solid #ff8800;color:#ff8800;border-radius:4px;padding:1px 5px;font-size:9px;">🧅 TOR</span>`:''}
        <button onclick="navigator.clipboard.writeText('${p.ip}').then(()=>this.textContent='✅');setTimeout(()=>this.textContent='📋',1500)"
          style="background:${t.bg};border:1px solid ${t.border}33;color:${t.dim};cursor:pointer;font-size:10px;border-radius:4px;padding:2px 6px;">📋</button>
      </div>
    </div>
    <div style="color:${vpnColor};font-size:11px;margin-bottom:4px;">${isTor?'🧅 Tor Exit Node':p.type}${settings.showCandType.val?` <span style="color:${t.dim};">• ${p.candType}</span>`:''}</div>
    ...
    <div style="color:${t.dim};font-size:10px;margin-top:4px;">
      ${settings.showTimestamp.val?`🕐 ${p.time}`:''}
      ${settings.showCoords.val?` • 🌐 ${p.loc}`:''}
      ${settings.showPostal.val?` • 📮 ${p.postal}`:''}
    </div>
    ${settings.showDuration.val?`<div id="embedDur_${p.ip.replace(/[:.]/g,'_')}" style="color:${t.dim};font-size:10px;margin-top:3px;">⏱️ Connected: 0s <span class="liveDot" style="color:${color};">●</span></div>`:''}
    ${settings.showTimeline.val?`<div style="height:2px;background:#111;border-radius:1px;margin-top:6px;overflow:hidden;"><div id="embedTl_${p.ip.replace(/[:.]/g,'_')}" style="height:100%;width:0%;background:${color};border-radius:1px;transition:width 0.5s;"></div></div>`:''}
  `;
  area.appendChild(card);

  const countEl=document.getElementById('embedPeerCount');
  if (countEl) {
    const current=parseInt(countEl.textContent.replace('Peers: ','')||'0');
    countEl.textContent=`Peers: ${current+1}`;
  }

  // Duration timer for embed
  if (settings.showDuration.val) {
    const startTime=Date.now();
    const durId=`embedDur_${p.ip.replace(/[:.]/g,'_')}`;
    const tlId=`embedTl_${p.ip.replace(/[:.]/g,'_')}`;
    setInterval(()=>{
      const el=document.getElementById(durId);const tl=document.getElementById(tlId);
      if (!el) return;
      const secs=Math.floor((Date.now()-startTime)/1000);
      const mins=Math.floor(secs/60);
      el.innerHTML=`⏱️ Connected: ${mins>0?mins+'m ':''}${secs%60}s <span class="liveDot" style="color:${color};">●</span>`;
      if (tl) tl.style.width=Math.min(100,(secs/300)*100)+'%';
    },1000);
  }

  // Scroll embed area to bottom
...
function enableEmbedMode() {
  ...
  if (!chatWindow) { alert('Could not find the chat window on this page. Make sure you\'re on umingle.com/video/'); return; }

  embeddedMode=true;
  document.getElementById('ppEmbedBtn').textContent='✅ Embedded';
  document.getElementById('ppEmbedBtn').style.background='linear-gradient(135deg,#003300,#005500)';
  document.getElementById('ppEmbedBtn').style.borderColor='#00ff88';
  ...
  document.getElementById('embedCopyAll').addEventListener('click',()=>{
    const text=peerLog.map((p,i)=>[
      ...
      `City: ${p.city}, ${p.region}`,`ISP:  ${p.org}`,`Time: ${p.time}`,
    ].join('\n')).join('\n\n');
    navigator.clipboard.writeText(text||'No peers yet').then(()=>{
      const btn=document.getElementById('embedCopyAll');btn.textContent='✅';
  ...
  document.getElementById('embedClose').addEventListener('click',()=>{
    const el=document.getElementById('frostEmbedContainer');
    if (el) el.remove();
    embeddedMode=false;
    document.getElementById('ppEmbedBtn').textContent='🔗 Embed';
    document.getElementById('ppEmbedBtn').style.background='linear-gradient(135deg,#1a0f40,#2a1a60)';
    document.getElementById('ppEmbedBtn').style.borderColor='#7b68ee';
    embedMapLoaded=false;embedLeafletMap=null;
  ...
  // Add any already-captured peers
...
// ---- Leaflet map (float panel) ----
let mapLoaded=false,leafletMap=null;
const mapMarkers=[];let currentMapMarker=null;
...
function initMap() {
  ...
  const script=document.createElement('script');script.src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js';
  script.onload=()=>{
    const mapEl=document.getElementById('frostMap');mapEl.style.height='100%';
...
function addMapMarker(p, fly=true) {
  ...
  if (isNaN(lat)||isNaN(lon)) return;
  const color=getMarkerColor(p);
  const icon=L.divIcon({className:'',html:`<div style="width:16px;height:16px;border-radius:50%;background:${color};border:2px solid #fff;box-shadow:0 0 10px ${color},0 0 20px ${color}44;"></div>`,iconSize:[16,16],iconAnchor:[8,8]});
...
function updateStats() {
  ...
  const bar=(pct,color)=>`<div style="height:6px;background:#111;border-radius:3px;overflow:hidden;margin-top:3px;"><div style="width:${pct}%;height:100%;background:${color};border-radius:3px;transition:width 0.5s;"></div></div>`;
  el.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px;">
      ${[['Total',total,'#c8b8ff'],['VPN/DC',sessionStats.vpn,'#ff4444'],['Residential',sessionStats.residential,'#7b68ee'],['Mobile',sessionStats.mobile,'#00bfff'],['Hosting',sessionStats.hosting,'#ffaa00'],['Tor',sessionStats.tor,'#ff8800']].map(([l,v,c])=>`<div style="background:#0a0818;border:1px solid #1a1030;border-radius:8px;padding:10px;"><div style="font-size:10px;color:#443366;">${l}</div><div style="font-size:20px;font-weight:700;color:${c};margin-top:2px;">${v}</div></div>`).join('')}
    </div>
    <div style="background:#0a0818;border:1px solid #1a1030;border-radius:8px;padding:12px;margin-bottom:8px;">
      ...
      ${[['🔴 VPN/DC',vpnPct,'#ff4444'],['🟢 Residential',resPct,'#7b68ee'],['📱 Mobile',mobPct,'#00bfff'],['🟡 Hosting',hostPct,'#ffaa00'],['🧅 Tor',torPct,'#ff8800']].map(([l,p,c])=>`<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;font-size:10px;"><span>${l}</span><span style="color:${c};">${p}%</span></div>${bar(p,c)}</div>`).join('')}
    </div>
    <div style="background:#0a0818;border:1px solid #1a1030;border-radius:8px;padding:12px;margin-bottom:8px;">
      <div style="font-size:10px;color:#443366;letter-spacing:1px;margin-bottom:8px;">TOP COUNTRIES</div>
      ${topC.map(([c,n])=>`<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #161616;font-size:11px;"><span>${c}</span><span style="color:#7b68ee;font-weight:700;">${n}</span></div>`).join('')}
    </div>
    <div style="background:#0a0818;border:1px solid #1a1030;border-radius:8px;padding:12px;">
      <div style="font-size:10px;color:#443366;letter-spacing:1px;margin-bottom:8px;">TIMELINE</div>
      <div style="display:flex;flex-direction:column;gap:4px;max-height:140px;overflow-y:auto;">
        ${peerLog.map(p=>`<div style="display:flex;align-items:center;gap:6px;font-size:10px;"><div style="width:8px;height:8px;border-radius:50%;background:${p.type.includes('🔴')?'#ff4444':'#7b68ee'};flex-shrink:0;"></div><span style="color:#443366;">${p.time}</span><span style="color:#c8b8ff;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${p.ip}</span><span style="color:#443366;">${p.city||'?'}</span></div>`).join('')}
      </div>
    </div>
  `;
}
...
document.querySelectorAll('.frostToggle').forEach(wrap=>{
  wrap.addEventListener('click',()=>{
    const key=wrap.dataset.key;settings[key].val=!settings[key].val;
...
function applyTheme(t) {
  ...
  document.querySelectorAll('.frostToggle').forEach(w=>{const on=settings[w.dataset.key].val;w.style.background=on?t.border:'#1e1e1e';w.style.borderColor=on?t.border:'#333';});
  ...
  document.querySelectorAll('.themeBtn').forEach(b=>{
    const active=themes[b.dataset.theme]===t;
...
// ---- Snap ----
function applySnap() {
  if (settings.snapToEdge.val){panel.style.transform='none';panel.style.top='10px';panel.style.left='auto';panel.style.right='10px';panel.style.maxHeight='calc(100vh - 20px)';}
}
...
document.getElementById('ppCopyAll').addEventListener('click',()=>{
  const text=peerLog.map((p,i)=>[`--- Peer #${i+1} ---`,`IP:      ${p.ip}`,`Type:    ${p.label} ${p.type}`,`City:    ${p.city}, ${p.region}`,`Country: ${p.country}`,`ISP:     ${p.org}`,`Coords:  ${p.loc}`,`Time:    ${p.time}`,`Duration:${p.duration||'active'}`].join('\n')).join('\n\n');
...
document.getElementById('ppExportBtn').addEventListener('click',()=>{
  ...
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`frosts_peers_${Date.now()}.json`;a.click();
});
...
document.getElementById('ppClear').addEventListener('click',()=>{
  ...
  // Clear embed too
  const embedArea=document.getElementById('embedPeerArea');
  if (embedArea){embedArea.innerHTML='<div id="embedEmpty" style="color:'+currentTheme.dim+';text-align:center;padding:16px 0;font-size:11px;">⏳ Waiting for peer connection...</div>';}
  const embedCount=document.getElementById('embedPeerCount');if(embedCount)embedCount.textContent='Peers: 0';
  embedMapMarkers.forEach(m=>embedLeafletMap&&embedLeafletMap.removeLayer(m));embedMapMarkers.length=0;embedCurrentMarker=null;
...
function addToPanel(p) {
  ...
  entry.style.cssText=`border:1px solid ${currentTheme.dim}44;border-left:3px solid ${color};border-radius:clamp(8px,2.5vw,12px);padding:clamp(8px,2.5vw,12px);margin-bottom:clamp(6px,2vw,9px);background:${currentTheme.header}88;`;
  ...
  let html=`
    ...
    <div style="color:${vpnColor};font-size:clamp(10px,2.5vw,11px);margin-bottom:4px;">${isTor?'🧅 Tor Exit Node':p.type}${settings.showCandType.val?` <span style="color:${currentTheme.dim};">• ${p.candType}</span>`:''}</div>
  `;
  if (!settings.compactMode.val) {
    ...
    if (settings.showDuration.val) html+=`<div id="dur_${p.ip.replace(/[:.]/g,'_')}" style="color:${currentTheme.dim};font-size:10px;margin-top:3px;">⏱️ Connected: 0s <span class="liveDot" style="color:${color};">●</span></div>`;
  ...
  peerCount++;document.getElementById('ppCount').textContent=`Peers: ${peerCount}`;
...
async function geoIP(ip,port,candType) {
  ...
  try {
    ...
    const p={
      ...
      city:d.city||'?',region:d.region||'?',country:d.country||'?',org:d.org||'Unknown',
...
console.log('%c[🐉 Frosts Tools v0.1.4 Ready]','color:#7b68ee;font-weight:bold;font-size:14px;');
